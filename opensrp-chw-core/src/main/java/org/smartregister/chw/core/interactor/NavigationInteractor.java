    ...
    protected int getCount(String tableName) {
        switch (tableName.toLowerCase().trim()) {
            ...
            case org.smartregister.chw.lab.util.Constants.TABLES.LAB_TEST_REQUESTS:
                ...
                //here we create a new case for rendering the count of the fields from the model or table
            case CoreConstants.TABLE_NAME.GENDER_EQUALITY:
                String sqlGe =
                        "SELECT count(*) " +
                                "   from " + CoreConstants.TABLE_NAME.GENDER_EQUALITY+ " ege " +
                                "   where ege.is_closed is 0 ";
